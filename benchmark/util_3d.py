...
class Instance(object):
    ...
    def __init__(self, mesh_vert_instances, instance_id):
        ...
        self.raw_inst_id = self.instance_id - self.label_id * 1000 - 1
    ...
    def to_dict(self):
        ...
        dict["raw_inst_id"] = self.raw_inst_id
...
def get_instances(ids, class_ids, class_labels, id2label, 
    ...
    for id in instance_ids:
        ...
        if inst.label_id in class_ids:
            ...
            if all_gt_axis is not None and all_gt_origin is not None:
                inst_articulation_i = id
                if id not in all_gt_axis:
                    # print("id: ", id)
                    # print("instance_ids: ", instance_ids)
                    # print("inst_ids: ", list(all_gt_axis.keys()))
                    inst_dict['axis'] = np.array([0, 0, 1])
                    inst_dict['origin'] = np.array([0, 0, 0])
                else:
                    inst_dict['axis'] = all_gt_axis[inst_articulation_i]
                    inst_dict['origin'] = all_gt_origin[inst_articulation_i]
            if interaction_labels is not None:
                inst_dict['interaction_mask'] = interaction_labels == inst_dict['raw_inst_id']
                inst_dict['mov_mask'] = ids == id
                if inst_dict['interaction_mask'].sum() == 0:
                    debug = None
