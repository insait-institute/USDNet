...
class SceneParser:
    ...
    def get_data(self, ignore_index=0):
        ...
        mov_gt = np.ones(num_points, dtype=np.int32) * ignore_index
        ...
        for articulation in self.articulation_parts:
            ...
            if mov_id in self.articulation_params:
                ...
                if len(indices) != 0:
                    inter_gt[indices] = mov_id + 1000 * sem_id
                ...
                if self.interaction_as_movement:
                    ...
                    for trace_id in trace_list:
                        ...
                        if len(indices) != 0:
                            mov_mask[indices] = True
                mov_gt[mov_mask] = mov_id + 1000 * sem_id
...
class Articulate3DPreprocessing(BasePreprocessing):
    ...
    def process_file(self, folderpath, mode):
        ...
        coords, colors, normals, mov_gt, inter_gt = scene_parser.get_data()

        points = np.hstack([coords, colors, normals, mov_gt[..., None], inter_gt[..., None]])
        ## save points
        processed_filepath = (
